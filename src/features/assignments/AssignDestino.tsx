...
import { fetchHolders, fetchAssignments } from '../../data/api'
import { useAssignmentDraft } from '../../store/assignmentDraft'

export function AssignDestino() {
  const navigate = useNavigate()
  const { setDestination, setToast } = useAssignmentDraft()

  const { data: allHolders = [] } = useQuery({ queryKey: ['holders'], queryFn: fetchHolders })
  const { data: myAssignments = [] } = useQuery({ queryKey: ['assignments'], queryFn: () => fetchAssignments() })

  const busyIds = new Set(
    myAssignments.filter((a) => a.status === 'enTransito').map((a) => a.destinationHolderId),
  )
  const eligible = allHolders.filter((h) => h.role === 'executive' && !busyIds.has(h.id))
